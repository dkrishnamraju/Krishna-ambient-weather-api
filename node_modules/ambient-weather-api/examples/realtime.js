...
function processData()
{
  ...
  /* if new data received between 1 AM to 6 AM*/
  if((now.hour() >= 1) && (now.hour() < 6))  
  {
    ...
    if(newWeatherData == 1)
    {
      if(now.minute() % 3 === 0 )
      {
        ...
        if((now.hour() >= 1) && (now.hour() < 6))
        {
          if((AvgWindSpeedKmph < 1))
          {        TurnOnAeratorsFlag = 1; }
          if((AvgWindSpeedKmph > 2))
      ...
      //##########################################################################################
        // 10 PM to 1 AM 
        if(((now.hour() >= 22) && (now.hour() < 24)) || ( (now.hour() >= 0 && now.hour() < 1)))
        {
          if((AvgWindSpeedKmph < 0.1))
          {        TurnOnAeratorsFlag = 1; }
          if((AvgWindSpeedKmph > 1))
          {        TurnOnAeratorsFlag = 0; }
        }
      ...
      //console.log(convertToIST(data.date) + ' - ' + getName(data.device) + ' current outdoor temperature is: ' + fahrenheitToCelsius(data.tempf) + '°C');
